...
public final class BonsaiBlockEntity extends BlockEntity {
    ...
    private @Nullable ResourceLocation trunkBlockId;
    private @Nullable ResourceLocation leavesBlockId;
    ...
    public @Nullable ResourceLocation getTrunkBlockId() {
        return trunkBlockId;
    }

    public @Nullable ResourceLocation getLeavesBlockId() {
    ...
    protected void loadAdditional(CompoundTag input, HolderLookup.Provider registries) {
        ...
        this.dead = input.getBoolean(DEAD_KEY);
        this.shape = Shape.fromName(input.contains(SHAPE_KEY) ? input.getString(SHAPE_KEY) : Shape.SEMI_CASCADE.getSerializedName());
        ResourceLocation trunk = ResourceLocation.tryParse(input.getString(TRUNK_KEY));
        ResourceLocation leaves = ResourceLocation.tryParse(input.getString(LEAVES_KEY));
        this.trunkBlockId = trunk != null && isRegisteredBlock(trunk) ? trunk : null;
        this.leavesBlockId = leaves != null && isRegisteredBlock(leaves) ? leaves : null;
        this.plantStorage.setStoredStack(input.contains(PLANT_SLOT_KEY, 10)
                ? ItemStack.parse(registries, input.getCompound(PLANT_SLOT_KEY)).orElse(ItemStack.EMPTY).copyWithCount(1)
                : ItemStack.EMPTY);
    ...
    protected void saveAdditional(CompoundTag output, HolderLookup.Provider registries) {
        ...
        if (!storedPlant.isEmpty()) {
            output.put(PLANT_SLOT_KEY, storedPlant.save(registries));
        }
    }
    ...
    private static @Nullable BlockState getStoredBlockState(@Nullable ResourceLocation id) {
        if (id == null) {
            return null;
        }
