...
public final class IroriFuelState {
    ...
    void load(CompoundTag input, HolderLookup.Provider registries) {
        firewoodModel = parseFirewoodModel(input.getString(FIREWOOD_MODEL_KEY)).orElse(null);
        burnTime = input.getInt(BURN_TIME_KEY);
        burnTimeTotal = input.contains(BURN_TIME_TOTAL_KEY) ? input.getInt(BURN_TIME_TOTAL_KEY) : burnTime;
        burnCycle = input.getInt(BURN_CYCLE_KEY);
        ashState = input.getBoolean(ASH_STATE_KEY);
        items = emptyItems();
        ContainerHelper.loadAllItems(input, items, registries);
        if (items.getFirst().isEmpty()) {
            if (input.contains(FUEL_STACK_KEY, 10)) {
                ItemStack.parse(registries, input.getCompound(FUEL_STACK_KEY)).ifPresent(stack -> items.set(0, stack));
            }
        }
    }
    ...
    private void consumeFuel() {
        ...
        fuelStack.shrink(1);
        if (fuelStack.isEmpty()) {
            items.set(0, fuelItem.hasCraftingRemainingItem()
