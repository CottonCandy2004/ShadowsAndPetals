...
public abstract class ItemAccessResourceHandler<T> implements ResourceHandler<T> {
    ...
    @Override public int insert(int index,T resource,int amount,TransactionContext tx){if(amount<=0||!isValid(index,resource))return 0;int current=(int)getAmountAsLong(index),add=Math.min(amount,getCapacity(index,resource)-current);if(add>0)itemAccess.setResource(update(itemAccess.getResource(),index,resource,current+add));return add;}
    @Override public int extract(int index,T resource,int amount,TransactionContext tx){if(amount<=0||!isValid(index,resource))return 0;int current=(int)getAmountAsLong(index),out=Math.min(amount,current);if(out>0)itemAccess.setResource(update(itemAccess.getResource(),index,resource,current-out));return out;}
}