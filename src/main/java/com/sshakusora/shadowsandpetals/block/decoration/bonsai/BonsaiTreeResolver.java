 ...
 * can also be supplied by another mod through {@link #register(ResourceLocation,
 * ResourceLocation, ResourceLocation)}. When no explicit mapping exists, the server-side
...
public final class BonsaiTreeResolver extends SimpleJsonResourceReloadListener {
    private static final Logger LOGGER = LogUtils.getLogger();
    private static final Gson GSON = new Gson();
    private static final ResourceLocation DEFAULT_LEAVES = ResourceLocation.withDefaultNamespace("oak_leaves");
    ...
    public static final Codec<Definition> DEFINITION_CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ResourceLocation.CODEC.fieldOf("sapling").forGetter(Definition::sapling),
            ResourceLocation.CODEC.fieldOf("trunk").forGetter(Definition::trunk),
            ResourceLocation.CODEC.optionalFieldOf("leaves").forGetter(Definition::leaves)
    ).apply(instance, Definition::new));

    private static final BonsaiTreeResolver INSTANCE = new BonsaiTreeResolver();
    private static final Map<ResourceLocation, Definition> API_MAPPINGS = new ConcurrentHashMap<>();
    private static final Set<ResourceLocation> WARNED_FALLBACKS = ConcurrentHashMap.newKeySet();
    private static final Set<ResourceLocation> WARNED_INFERENCE = ConcurrentHashMap.newKeySet();
    ...
    private volatile Map<ResourceLocation, Definition> dataMappings = Map.of();

    private BonsaiTreeResolver() {
        super(GSON, "bonsai_trees");
    }
     ...
     */
    public static void register(ResourceLocation sapling, ResourceLocation trunk, @Nullable ResourceLocation leaves) {
        API_MAPPINGS.put(
    ...
    protected void apply(
        ...
        Map<ResourceLocation, Definition> bySapling = new HashMap<>();
        for (Map.Entry<ResourceLocation, JsonElement> entry : loaded.entrySet()) {
            Definition definition = DEFINITION_CODEC.parse(JsonOps.INSTANCE, entry.getValue())
                    .resultOrPartial(error -> LOGGER.error("Could not parse bonsai mapping {}: {}", entry.getKey(), error))
                    .orElse(null);
            if (definition == null) {
                continue;
            }
    ...
    private static @Nullable Result resolveExplicit(ResourceLocation saplingId) {
    ...
    private static Result fallback(ResourceLocation saplingId) {
    ...
    private static @Nullable Result inferFromTreeGrower(
        ...
        TreeGrower grower = treeGrower(sapling);
        if (grower == null) {
            return null;
        }
        ...
        for (int i = 1; i < candidates.size(); i++) {
            ...
            if (!selected.equals(candidate)) {
                ResourceLocation saplingId = BuiltInRegistries.BLOCK.getKey(sapling);
    ...
    private static List<Optional<ResourceKey<ConfiguredFeature<?, ?>>>> featureKeys(
        ...
        return List.of(
                readFeatureKey(grower, "tree"),
                readFeatureKey(grower, "secondaryTree"),
                readFeatureKey(grower, "flowers"),
                readFeatureKey(grower, "secondaryFlowers"),
                readFeatureKey(grower, "megaTree"),
                readFeatureKey(grower, "secondaryMegaTree")
        );
    }
    ...
    private static @Nullable Block sampleProvider(
        ...
        for (int i = 0; i < PROVIDER_SAMPLES; i++) {
            try {
                BlockState state = provider.getState(RandomSource.create(seed + i), pos);
                if (!state.isAir()) {
    ...
    public record Definition(ResourceLocation sapling, ResourceLocation trunk, Optional<ResourceLocation> leaves) {
        public Definition {
            Objects.requireNonNull(sapling, "sapling");
            Objects.requireNonNull(trunk, "trunk");
    ...
    private static @Nullable Block getRegisteredBlock(ResourceLocation id) {
        Block block = BuiltInRegistries.BLOCK.get(id);
        return block == Blocks.AIR ? null : block;
    }

    private static @Nullable TreeGrower treeGrower(SaplingBlock sapling) {
        try {
            var field = SaplingBlock.class.getDeclaredField("treeGrower");
            field.setAccessible(true);
            return (TreeGrower) field.get(sapling);
        } catch (ReflectiveOperationException | RuntimeException exception) {
            LOGGER.debug("Could not read sapling tree grower", exception);
            return null;
        }
    }

    @SuppressWarnings("unchecked")
    private static Optional<ResourceKey<ConfiguredFeature<?, ?>>> readFeatureKey(
            TreeGrower grower,
            String name
    ) {
        try {
            var field = TreeGrower.class.getDeclaredField(name);
            field.setAccessible(true);
            return (Optional<ResourceKey<ConfiguredFeature<?, ?>>>) field.get(grower);
        } catch (ReflectiveOperationException | RuntimeException exception) {
            LOGGER.debug("Could not read tree grower feature {}", name, exception);
