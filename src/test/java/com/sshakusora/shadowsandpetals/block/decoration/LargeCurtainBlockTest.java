...
class LargeCurtainBlockTest {
    ...
    void everyDyeColorHasACompleteStateMatrixAndItemModel() throws IOException {
        for (DyeColor color : DyeColor.values()) {
            ...
            assertEquals(256, variants.entrySet().size(), id);
            ...
            JsonObject item = loadJson("assets/shadowsandpetals/models/item/" + id + ".json");
    ...
    void generatedStateMappingUsesSideAndQuadrantSpecificModels() throws IOException {
        ...
        assertEquals(
                ...
                variants.getAsJsonObject("anchor=true,animating=false,column=outer,facing=north,half=lower,open=false,side=left")
                        .get("model").getAsString()
        );
        assertEquals(
                "shadowsandpetals:block/large_curtain/static/right/open/white/upper_inner",
                variants.getAsJsonObject("anchor=false,animating=false,column=inner,facing=north,half=upper,open=true,side=right")
                        .get("model").getAsString()
        );
    }
